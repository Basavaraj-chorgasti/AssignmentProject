...
class App extends Component{
  ...
  componentDidMount(){
    ...
    .then(result=>{
      console.log(result)
       this.setState({posts : result.data.data.children})
       console.log(this.state.posts)
       this.action();
      //  this.state.posts.map(post => console.log(htmlDecode(post.data.selftext_html)))
  ...
  action=()=>{
    ...
    for(var i=0;i<posts.length;i++)
    {
      ...
      // console.log(url)
      var selftext_html = posts[i].data.selftext_html;
     var self = this.htmlDecode(selftext_html);
     var html = '<div class="col-sm-6"><div class= "ss" style="border: 2px solid black;"><h3>'+title+'</h3><div><b>Score : '+score+'</b></div>'+self+'<div> Site URl:<a href='+'"'+url+'"'+'>'+url+'</a></div></div></div>';
    document.getElementById("hiv").insertAdjacentHTML("beforeend",html);
  ...
  htmlDecode=(input)=> {
    ...
    // document.getElementById("h").insertAdjacentHTML("beforeend",doc.documentElement.textContent );
    // this.Display(doc.documentElement.textContent);
    return doc.documentElement.textContent;
  }
  // Display=(input)=>{
  //   var html = input+'<div>-----------------------</div>';
  //   console.log(html)
  //   document.getElementById("hiv").insertAdjacentHTML("beforeend",html);
  // }
  render(){
